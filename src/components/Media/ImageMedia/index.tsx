...
export const ImageMedia: React.FC<MediaProps> = ({
  ...
  let src: string | StaticImageData | null = srcProp || null
  let alt = altProp || ''
  ...
  if (!src && resource && typeof resource === 'object') {
    ...
    width = resourceWidth ?? undefined
    height = resourceHeight ?? undefined
    alt = resourceAlt || alt
  }

  // ✅ Return nothing if src is still not resolved
  if (!src) return null

  const resolvedWidth = !fill ? (width ?? 1200) : undefined
  const resolvedHeight = !fill ? (height ?? 800) : undefined

  const loading = loadingProp || (!priority ? 'lazy' : undefined)
  ...
  return (
    <picture className={cn(pictureClassName)}>
      <NextImage
        ...
        fill={fill}
        width={resolvedWidth ? resolvedWidth / 2 : undefined}
        height={resolvedHeight ? resolvedHeight / 2 : undefined}
        placeholder="blur"
